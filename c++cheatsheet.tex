...
// put back a char to (*<cin>*) nextChar will be the next char read by cin.get()
...
public:
    ...
    Car(double size);
    // mutators
    void setEngineSize(const double size);
    // accessors
...
Car::Car(double size) : engineLiter(size)
{
}

// parameter passed by reference for efficiency
void Car::setEngineSize(const double size)
{
    engineLiter = size;
}
...
{
    return car1.engineLiter == car2.engineLiter;
}
\end{lstlisting}
%
...
void myFunction( ) throw (DivideByZero, OtherException);
...
class AList
{
    ...
    AList(AList<T>& b);
    // assignment operator
    AList<T>& operator=(const AList<T>& b);
    // friend insertion operator
    template <class TT>
    friend ostream& operator<<(ostream& outs, const AList<TT>& rhs);
private:
    T *p;
    int size;
}
