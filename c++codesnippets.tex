...
\date{\small \today~-~Version 2.01~-~GNU GPL v3.0}

%\layout

\maketitle

% We don't want page numbers on the following page
\thispagestyle{empty}
...
smatch results;
string zip("The string NJ07936-3173"
    " and NJ07936-3175 are ZIP codes");
...
regex regex{ R"((\w{2})(\d{5})(-\d{4})?)" };(*\index{\texttt{regex}}*)(*\index{Strings!raw format}*)
smatch results;
string zip("The string NJ07936-3173 and"
    " NJ07936-3175 are ZIP codes");
...
for (auto& dir_entry : dir) {
    if ( dir_entry.is_directory() ) {
        ...
        cout << dir_entry.file_size() << " bytes big ";
        cout << endl;(*\index{Utilities!Filesystem!directory entry file size}*)
    }
}
\end{lstlisting}
\begin{enumerate}
...
auto one_res =
    async(launch::async,incrementing_thread<'1'>,6);
auto two_res =
    async(launch::async,incrementing_thread<'2'>,9);
...
auto one_res =
    async(launch::async,
        atomic_incrementing_thread<'1'>,6);
auto two_res =
    async(launch::async,
        atomic_incrementing_thread<'2'>,9);
