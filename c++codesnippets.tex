...
It is introduced by \texttt{[ ]} which are called \emph{lambda introducers}.\index{[ ]}
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Disabling copy constructors and assignment\\ \\ Notice the \texttt{= delete}\index{= delete}  syntax for disabling them. If they were allowed, slicing\index{Slicing} might occur when derived objects are copied into base objects. Usually, \texttt{sizeof(Shape) <= sizeof(derived classes from Shape)}. By allowing copying, some attributes are not be copied, which might lead to crashes when member functions of the derived classes are called! Note that slicing is the class object equivalent of integer truncation.
...
protected:
    ...
    // derived classes
    virtual void draw_shape() = 0;(*\index{= 0}*)
    virtual void move_shape(int dx, int dy) = 0;(*\index{= 0}*)
    ...
};
\end{lstlisting}
\begin{enumerate}
