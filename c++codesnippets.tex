...
\date{\small \today~-~Version 1.44~-~GNU GPL v3.0}
...
\small
\begin{enumerate}
\item[$\Rightarrow$] Example class using dynamic arrays:\index{Classes!example of a vector class}
\end{enumerate}
\begin{lstlisting}
class MyVector
{
public:
    ...
    // move constructor
    MyVector(MyVector&&);
    // copy assignment(*\index{\textbf{operator=}}*)
    MyVector& operator=(const MyVector&);
    // move assignment
    MyVector& operator=(MyVector&&);
    // virtual destructor(*\index{Classes!virtual destructor}*)
    virtual ~MyVector() { if (!e) delete[] e; }
    // subscript operators(*\index{Classes!subscript operator}*)
    double& operator[](size_t i) { return e[i]; }(*\index{\textbf{operator[]}}*)
    double& operator[](size_t i) const { return e[i]; };
    // size (constant member function)(*\index{Classes!constant member function}*)
...
If you give at least one constructor, then the compiler will generate no other constructors.\index{Classes!constructors}
\end{enumerate}
\begin{lstlisting}
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Copy constructor. Note: Argument is passed by const reference, i.e. no copies and no changes.\index{Classes!copy constructor}
\end{enumerate}
\begin{lstlisting}
// copy constructor
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Copy assignment\index{Classes!copy assignment}
\end{enumerate}
\begin{lstlisting}
// copy assignment
