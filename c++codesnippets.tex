...
\date{\small \today~-~Version 1.69~-~GNU GPL v3.0}
...
\small
\begin{enumerate}
\item[$\Rightarrow$] The first argument of a \texttt{static\_assert} is a constant expression that must be true:
\end{enumerate}
\begin{lstlisting}
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] \texttt{constexpr} applied to functions instructs the compiler to try to evaluate the function at compile time:\index{Constants!constant functions}
\end{enumerate}
\begin{lstlisting}
...
\small
\begin{enumerate}
\item[$\Rightarrow$] Use \texttt{static\_cast} for normal casting\index{Type casting!\textbf{static\_cast}}, i.e. types that can be converted into each other:
\end{enumerate}
\begin{lstlisting}
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Use \texttt{reinterpret\_cast} for casting between unrelated pointer types\index{Type casting!\textbf{reinterpret\_cast}}:
\end{enumerate}
\begin{lstlisting}
