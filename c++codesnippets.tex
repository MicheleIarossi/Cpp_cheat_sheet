...
\date{\small \today~-~Version 1.56~-~GNU GPL v3.0}
...
\small
\begin{enumerate}
\item[$\Rightarrow$] Use \texttt{static\_cast} for normal casting\index{Casts!\textbf{static\_cast}}\index{\textbf{static\_cast}}, i.e. types that can be converted into each other:
\end{enumerate}
\begin{lstlisting}
...
// back to (*double*) type
double *pd = static_cast<double*>(p);(*\index{\textbf{double}}*)
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Use \texttt{reinterpret\_cast} for casting between unrelated pointer types\index{Casts!\textbf{reinterpret\_cast}}\index{\textbf{reinterpret\_cast}}:
\end{enumerate}
\begin{lstlisting}
// reinterprets a (*long*) value as a (*double*) one
long n = 53;(*\index{\textbf{long}}*)
double *pd = reinterpret_cast<double *>(&n);(*\index{\textbf{double}}*)
...
It is introduced by \texttt{[ ]} which are called \emph{lambda introducers}.\index{\texttt{[ ]}}
...
\small
\begin{enumerate}
\item[$\Rightarrow$] Input stream \texttt{cin}\index{\texttt{cin}}, output stream \texttt{cout}\index{\texttt{cout}}, error stream \texttt{cerr}\index{\texttt{cerr}}:
...
public:
    ...
    // virtual destructor
    virtual ~MyVector() { if (e) delete[] e; }(*\index{Classes!virtual destructor}*)(*\index{\textbf{delete}}*)
...
public:
    ...
    Shape(const Shape&) = delete;(*\index{\textbf{const}}*)
    // no copy assignment allowed
    Shape& operator=(const Shape&) = delete;(*\index{\textbf{const}}*)
    // virtual destructor
    virtual ~Shape() {}
...
protected:
    ...
    virtual void move_shape(int dx, int dy) = 0;(*\index{\textbf{int}}*)
    // protected setter methods
    virtual void set_color_shape(Color_type c) {
        new_color = to_fl_color(c);
    }
    virtual void set_color_shape(int c) {(*\index{\textbf{int}}*)
        new_color = to_fl_color(c);
    }
    virtual void set_style_shape(Style_type s, int w);(*\index{\textbf{int}}*)
    virtual void set_font_shape(Font_type f, int s);(*\index{\textbf{int}}*)
...
\end{lstlisting}
\begin{enumerate}
\item[$\Rightarrow$] Disabling copy constructors and assignment\\ \\ Notice the \texttt{= delete}\index{\texttt{= delete}}  syntax for disabling them. If they were allowed, slicing\index{Slicing} might occur when derived objects are copied into base objects. Usually, \texttt{sizeof(Shape) <= sizeof(derived classes from Shape)}. By allowing copying, some attributes are not be copied, which might lead to crashes when member functions of the derived classes are called! Note that slicing is the class object equivalent of integer truncation.
...
protected:
    ...
    // derived classes
    virtual void draw_shape() = 0;(*\index{\texttt{= 0}}*)
    virtual void move_shape(int dx, int dy) = 0;(*\index{\texttt{= 0}}*)(*\index{\textbf{int}}*)
    ...
};
\end{lstlisting}
\begin{enumerate}
...
\texttt{throw}\index{\textbf{throw}} can be of any type.
\end{enumerate}
\begin{lstlisting}
// exception class
...
public:
    ...
    MyVector& operator=(MyVector&&);
    // virtual destructor
    virtual ~MyVector() { if (e) delete[] e; }(*\index{\textbf{delete}}*)
    // subscript operators
    // write
...
template<class T, class A=allocator<T>>(*\index{\texttt{allocator}}*)
...
\small
\begin{enumerate}
\item[$\Rightarrow$] Sequential containers: \texttt{list}\index{Containers!sequential!\texttt{list}}\index{Containers!sequential!\texttt{vector}|see{Vector}}
\end{enumerate}
\begin{lstlisting}
...
long sl = accumulate(v.begin(),v.end(),long(0));(*\index{Algorithms!\texttt{accumulate}}*)(*\index{\textbf{long}}*)
...
double prod = accumulate(v.begin(),v.end(),1.0,multiplies<int>());(*\index{\textbf{double}}*)
